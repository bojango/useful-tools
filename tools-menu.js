(() => {
  ...
  style.textContent = `
    ...
    .tools-menu-panel{position:absolute;right:0;bottom:46px;width:210px;border:1px solid #263a34;background:#070c0b;box-shadow:0 10px 30px rgba(0,0,0,.35);display:none}
    ...
    @media(max-width:520px){.tools-menu-wrap{right:10px;bottom:10px}.tools-menu-panel{width:min(210px,calc(100vw - 20px))}}
  `;
  document.head.appendChild(style);

  const path = location.pathname.replace(/\/+$/, '');
  const onPassword = /\/password$/i.test(path);
  const base = onPassword ? '..' : '.';
  ...
  wrap.innerHTML = `
    <div id="toolsMenuPanel" class="tools-menu-panel" aria-hidden="true">
      ...
      <a class="tools-menu-link" href="${base}/" ${!onPassword ? 'aria-current="page"' : ''}>
